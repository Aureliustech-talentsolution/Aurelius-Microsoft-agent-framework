...
class LifecycleEventEmitter:
    ...
    Example:
        ...
        ...     AgentLifecycleEventType.CREATED, lambda event: print(f"Agent {event.agent_name} created")
        ... )
    ...
    def register_listener(
        ...
        Example:
            ...
            >>> emitter.register_listener(AgentLifecycleEventType.CREATED, on_created)
        """
    ...
    async def emit(self, event: AgentLifecycleEvent) -> None:
        ...
        if event.event_type not in self._listeners:
            logger.debug("No listeners registered for event type", extra={"event_type": event.event_type.value})
            return
        ...
        for callback in self._listeners[event.event_type]:
            try:
                if asyncio.iscoroutinefunction(callback):
                    ...
                    task = asyncio.create_task(asyncio.to_thread(callback, event))
                    tasks.append(task)
...
def register_lifecycle_listener(
    ...
    Example:
        ...
        >>> register_lifecycle_listener(AgentLifecycleEventType.CREATED, on_agent_created)
    """
